...
def init_browser_id():
    """브라우저 localStorage에서 기기ID, 비밀번호, API키 불러오기"""
    # 이미 초기화되었으면 스킵
    if 'browser_init_done' in st.session_state:
        return

    st.session_state['browser_init_done'] = True

    # streamlit-javascript로 localStorage에서 모든 데이터 불러오기
    if BROWSER_ID_AVAILABLE:
        try:
            # 기기 ID
            saved_id = st_javascript("localStorage.getItem('writey_device_id')")
            if saved_id and saved_id != 0 and saved_id != "null":
                st.session_state['browser_device_id'] = saved_id

            # 비밀번호
            saved_pw = st_javascript("localStorage.getItem('writey_password')")
            if saved_pw and saved_pw != 0 and saved_pw != "null":
                st.session_state['saved_password'] = saved_pw

            # API 키
            saved_api = st_javascript("localStorage.getItem('writey_api_key')")
            if saved_api and saved_api != 0 and saved_api != "null":
                st.session_state['saved_api_key'] = saved_api
        except:
            pass
...
def save_password_to_browser(password):
    """비밀번호를 브라우저 localStorage에 저장"""
    if BROWSER_ID_AVAILABLE:
        try:
            st_javascript(f"localStorage.setItem('writey_password', '{password}')")
        except:
            pass
...
def save_api_key_to_browser(api_key):
    ...
    if BROWSER_ID_AVAILABLE:
        try:
            st_javascript(f"localStorage.setItem('writey_api_key', '{api_key}')")
        except:
            pass
...
with st.sidebar:
    ...
    if 'api_key' not in st.session_state:
        # 브라우저 localStorage에서 API 키 불러오기
        saved_api = get_saved_api_key()
        st.session_state['api_key'] = saved_api if saved_api else ''
    ...
    if st.session_state['show_api_section']:
        ...
        if api_key != st.session_state['api_key']:
            ...
            save_api_key_to_browser(api_key)  # 브라우저에 저장
