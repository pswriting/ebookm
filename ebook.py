...
def get_device_id():
    ...
    # 브라우저 ID가 없으면 None 반환 (인증 거부됨)
    return None

def init_browser_id():
    """브라우저 ID 초기화 (앱 시작 시 호출)"""
    # 이미 초기화되었으면 스킵
    if 'browser_device_id' in st.session_state and st.session_state['browser_device_id']:
        return True

    # streamlit-javascript로 브라우저 고유 ID 생성
    if BROWSER_ID_AVAILABLE:
        try:
            browser_id = st_javascript("""
                (function() {
                    let id = localStorage.getItem('writey_device_id');
                    if (!id) {
                        id = 'browser_' + Math.random().toString(36).substr(2, 9) + '_' + Date.now().toString(36);
                        localStorage.setItem('writey_device_id', id);
                    }
                    return id;
                })()
            """)
            if browser_id and browser_id != 0:
                st.session_state['browser_device_id'] = browser_id
                return True
        except:
            pass
    return False
...
# 브라우저 ID 초기화 (앱 시작 시)
if not init_browser_id():
    st.markdown("""
    <div style="display:flex;justify-content:center;align-items:center;height:100vh;flex-direction:column;">
        <h2 style="color:#d4af37;">⏳ 초기화 중...</h2>
