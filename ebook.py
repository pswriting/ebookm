...
def get_device_id():
    ...
    # 2. 로컬 파일에서 확인
    saved = load_config().get('device_id', '')
    if saved:
        st.session_state['browser_device_id'] = saved
        return saved
    return None
...
def get_saved_password():
    """저장된 비밀번호 불러오기"""
    # 1. 세션 상태에서 확인
    if 'saved_password' in st.session_state and st.session_state['saved_password']:
        return st.session_state['saved_password']
    # 2. 로컬 파일에서 확인
    saved = load_config().get('saved_password', '')
    if saved:
        st.session_state['saved_password'] = saved
        return saved
    return None
...
def get_saved_api_key():
    """저장된 API 키 불러오기"""
    # 1. 세션 상태에서 확인
    if 'saved_api_key' in st.session_state and st.session_state['saved_api_key']:
        return st.session_state['saved_api_key']
    # 2. 로컬 파일에서 확인
    saved = load_config().get('api_key', '')
    if saved:
...
def save_password_to_browser(password):
    ...
    save_config({'saved_password': password})

def save_api_key_to_browser(api_key):
    """API 키 저장 (세션 + 로컬 파일 + 쿠키)"""
...
if 'authenticated' not in st.session_state:
    ...
    # 2. 저장된 비밀번호로 자동 로그인 시도
    elif get_saved_password() == CORRECT_PASSWORD:
        st.session_state['authenticated'] = True
        save_authenticated()  # 로컬 파일에도 인증 상태 저장
    # 3. JSONBin에서 기기 승인 여부 확인
    else:
        ...
        if not device_id:
            ...
            if st.button("🔓 관리자로 바로 접속", key="btn_admin_first"):
                if admin_pw_first == ADMIN_PASSWORD:
                    ...
                    st.session_state['authenticated'] = True
                    save_authenticated()  # 로컬 파일에 인증 상태 저장
...
defaults = {
    ...
    'interview_data': {},
    'author_name': '',
    # 이전에 생성한 컨셉 저장 (중복 방지용)
    'generated_concepts': load_config().get('generated_concepts', []),
}
...
with st.sidebar:
    ...
    if 'api_key' not in st.session_state:
        # 1. 로컬 파일에서 API 키 불러오기
        saved_api = get_saved_api_key()
        if saved_api:
            st.session_state['api_key'] = saved_api
        else:
            # 2. JSONBin에서 API 키 불러오기
            device_id = get_device_id()
            if device_id:
                settings = get_device_settings(device_id)
                st.session_state['api_key'] = settings.get('api_key', '')
    ...
    # API 키 발급 방법 안내
    st.markdown("### 📖 Claude API 키 발급 방법")
    st.markdown("""
    **🟣 1단계: Anthropic 회원가입**
    1. 아래 버튼 클릭 → **"Sign up"** → Google 계정 또는 이메일로 가입
    """)
    st.link_button("🔗 Anthropic 가입 페이지", "https://console.anthropic.com/", use_container_width=True)

    st.markdown("""
    **💳 2단계: 결제 수단 등록**
    1. 로그인 후 **Settings** → **Billing** → **Add payment method**
    2. 카드 등록 후 **Add credits**로 충전 ($5~10 추천)
    """)
    st.link_button("🔗 Billing 페이지 바로가기", "https://console.anthropic.com/settings/billing", use_container_width=True)

    st.markdown("""
    **🔑 3단계: API 키 발급**
    1. **API Keys** → **Create Key** → 이름 입력 → **Create Key**
    2. 생성된 키 복사 (sk-ant-api03-... 형식) → 위 입력창에 붙여넣기
    """)
...
[도입] 나의 실패담/솔직한 고백 (2~3문장)
- "저도 처음엔 몰랐습니다"
- 구체적 숫자로 시작 (예: "3번 실패하고 47만원 날렸습니다")
...
"솔직히 말하면, 저도 3번 실패했습니다.

47만원을 날리고 나서야 깨달았습니다. 방법이 틀렸던 거죠.

근데 하나를 바꿨더니 2주 만에 결과가 나왔습니다.

이 책엔 제가 실패하면서 알게 된 것들만 담았습니다.
뻔한 이론은 없습니다. 바로 써먹을 수 있는 것들만요.
...
자, 이제 시작합니다."""
        ...
        epilogue_text = None
        if interview_data:
            epilogue_prompt = f"""에필로그를 작성합니다.

[저자 정보 - 참고용]
- 분야: {interview_data.get('field', '')}
- 경력: {interview_data.get('experience_years', '')}
- 책 주제: {interview_data.get('topic', '')}
- 전하고 싶은 말: {interview_data.get('final_message', '')}
- 어려움/실패: {interview_data.get('struggle_story', '')}
...
저도 처음엔 막막했습니다. 3개월간 결과가 0이었거든요.
근데 하나를 바꿨더니 달라졌습니다.

이 책엔 그때 알았더라면 좋았을 것들만 담았습니다.
제가 실패하면서 배운 것들이요.

지금 당장 하나만 해보세요.
오늘 안에 첫 번째 챕터에서 배운 걸 적용해보세요.
...
def generate_outline_only(interview_data, progress_placeholder):
    ...
    try:
        ...
        # 이전에 생성한 컨셉 목록 (중복 방지)
        previous_concepts = st.session_state.get('generated_concepts', [])
        previous_concepts_text = ""
        if previous_concepts:
            previous_concepts_text = f"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🚨 이미 사용된 컨셉 (절대 사용 금지!)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{chr(10).join([f'• {c}' for c in previous_concepts[-20:]])}

위 컨셉들과 완전히 다른 새로운 컨셉을 만드세요.
같은 영어 약자, 비슷한 패턴 금지!
"""

        concept_prompt = f"""당신은 크몽/클래스101 베스트셀러 전자책 기획자입니다.
이 책만의 '고급스럽고 기억하기 쉬운 프레임워크'를 만들어야 합니다.
...
(1개)"""
        ...
        book_concept = ask_ai(concept_prompt, 0.9)
        st.session_state['book_concept'] = book_concept

        # 생성한 컨셉 저장 (중복 방지용)
        if book_concept:
            # 프레임워크 이름 추출
            lines = book_concept.split('\n')
            for i, line in enumerate(lines):
                if '프레임워크' in line or '시스템' in line or '공식' in line:
                    if i + 1 < len(lines) and lines[i + 1].strip():
                        concept_name = lines[i + 1].strip().strip('()').strip()
                        if concept_name and len(concept_name) > 2:
                            previous_concepts.append(concept_name)
                            st.session_state['generated_concepts'] = previous_concepts[-30:]  # 최근 30개만 유지
                            save_config({'generated_concepts': previous_concepts[-30:]})
                        break
...
- PART 2. 수익이 자동으로 굴러가는 P의 비밀
- PART 3. M으로 하나를 열 개로 복제하는 기술

예시) "상위노출 알고리즘" 컨셉이라면:
- PART 1. 네이버가 순위를 정하는 진짜 기준
- PART 2. 경쟁 없이 1등 하는 블랙 키워드 공략법
- PART 3. 한 번 올리면 계속 오르는 자동 상승의 비밀
...
- "PART 2. 상위 1%만 아는 숨겨진 알고리즘"
- "PART 3. 한 번 세팅하면 알아서 굴러가는 자동화 루프"
- "PART 4. 30일 안에 첫 결과를 만드는 실전 순서"
- "PART 5. 월 100에서 월 1000으로 가는 복리 성장 로드맵"

❌ 나쁜 예:
- "PART 1. 시작" (추상적)
- "PART 2. 제국을 건설하다" (부자연스러운 과장)
- "PART 3. 기초 이해" (설명서 같음)
- "PART 1. 잘못된 공식: 왜~" (콜론 금지)
    ...
    try:
        ...
        for ch in st.session_state['outline']:
            ...
            for sub in ch_data['subtopics']:
                ...
                content_prompt = f"""'{sub}'에 대해 글을 씁니다.
...
[도입] 후킹 (1~2문단) - 이번 글은: {current_hook_style}
...
def auto_generate_all(topic, progress_placeholder):
    ...
    try:
        ...
        if targets and targets.get('personas'):
            first_target = targets['personas'][0]
            persona = f"{first_target.get('name', '')} - {first_target.get('demographics', '')}"
...
def suggest_targets(topic):
    """크몽/클래스101 베스트셀러 타겟 분석 - 실제 구매자 페르소나"""
    prompt = f"""주제: {topic}

이 주제의 전자책을 실제로 결제할 핵심 타겟 3개를 분석해주세요.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 타겟 분석 원칙
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

"누구나"를 타겟으로 하면 아무도 사지 않는다.
"딱 이 사람"이 보면 "이거 나 얘기잖아" 느끼게 해야 한다.
...
[블로그 수익화 전자책]
• 타겟: "퇴사 준비 중인 32세 대기업 마케터"
• 상황: 월급 외 수입 필요, 퇴근 후 2시간 투자 가능
• 진짜 고민: "블로그 6개월 했는데 애드센스 수익 월 3만원"

[주식 투자 전자책]
• 타겟: "결혼자금 모으는 28세 중소기업 직원"
• 상황: 월급 280만원, 투자금 500만원
• 진짜 고민: "유튜브 따라 샀다가 -30% 손실"

[다이어트 전자책]
• 타겟: "출산 후 10kg 안 빠지는 35세 워킹맘"
• 상황: 운동할 시간 없음, 야식 끊기 어려움
• 진짜 고민: "남편이 예전 몸매 얘기할 때 상처받음"
...
{{
    "personas": [
        {{
            "name": "구체적인 타겟명 (나이+직업+상황)",
            "demographics": "연령대, 직업, 상황",
            "needs": "이 타겟이 이 책을 사는 이유",
            "pain_points": ["진짜 고민1 (구체적 숫자 포함)", "고민2", "고민3", "고민4", "고민5"],
            "real_pain": "입 밖으로 못 꺼내는 진짜 고민 1문장",
            "buying_trigger": "결제 버튼 누르게 만드는 한마디"
        }}
    ],
    "best_persona": "가장 구매 가능성 높은 타겟 번호와 이유"
}}"""
    return ask_ai(prompt, 0.7)


def analyze_pains_deep(topic, persona):
    """타겟의 진짜 고민 심층 분석 - 결제하게 만드는 페인포인트 발굴"""
    prompt = f"""주제: {topic}
타겟: {persona}

이 타겟이 "이거 완전 내 얘기잖아" 느끼게 할 고민을 분석해주세요.
...
[블로그 수익화]
• 표면: "애드센스 수익이 안 나요"
• 진짜: "6개월 썼는데 월 3만원... 이게 맞나 싶음"
• 감정: "회사 동료한테 블로그 한다고 말했는데 부끄러움"

[주식 투자]
• 표면: "어떤 종목을 사야 하나요"
• 진짜: "유튜브 따라 샀다가 -200만원 물림"
• 감정: "아내한테 투자했다고 말 못함"

[다이어트]
• 표면: "살이 안 빠져요"
• 진짜: "저녁마다 배달앱 켜는 나를 통제 못함"
• 감정: "옛날 사진 보면 자괴감"
...
{{
    "surface_pains": {{
        "pains": ["입으로 말하는 고민 (구체적 상황 포함)"],
        "내가_이렇게_말함": "타겟이 친구한테 하소연할 때 실제로 하는 말"
    }},
    "hidden_pains": {{
        "pains": ["차마 말 못하는 진짜 고민 (구체적 숫자 포함)"],
        "왜_말_못함": "이 고민을 숨기는 이유"
    }},
    "emotional_pains": {{
        "감정들": ["느끼는 감정 (부끄러움, 자괴감, 조급함 등)"],
        "트리거_상황": "이 감정이 터지는 구체적 상황"
    }},
    "failed_attempts": {{
        "시도한_것들": ["이미 해봤지만 실패한 방법들"],
        "실패_이유": "왜 안 됐는지 (타겟 시점에서)",
        "남은_좌절감": "이 경험 후 느끼는 감정"
    }},
    "dream_outcome": {{
        "이상적_결과": "달성하고 싶은 구체적 상태 (숫자 포함)",
        "원하는_기간": "얼마 안에 되길 원하는지",
        "삶의_변화": "이루면 뭐가 달라지는지"
    }},
    "buying_triggers": {{
        "결제_버튼_누르는_순간": "어떤 말을 들으면 바로 결제하는지",
        "망설이는_이유": ["결제 직전 드는 의심들"],
        "극복_방법": "이 의심을 어떻게 해소하는지"
    }},
    "killer_hook": "이 타겟이 멈칫할 한 문장 (마케팅용)"
}}"""
    return ask_ai(prompt, 0.6)


def analyze_competitor_reviews(topic):
    """경쟁 전자책 부정 리뷰 분석 - 시장의 빈틈 발굴"""
    prompt = f"""주제: {topic}

이 주제 관련 전자책/도서의 부정적 리뷰를 분석해서 시장의 빈틈을 찾아주세요.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 분석 목적
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

"기존 책들이 못 채워준 것"을 찾아서
"드디어 이런 책이 나왔네!" 반응을 만드는 것
...
[블로그 수익화 책]
• "이론만 있고 실제로 뭘 해야 하는지 모르겠음"
→ 빈틈: 단계별 실행 가이드 부재
→ 기회: "1일차: OO 하기, 2일차: XX 하기" 형식

[주식 투자 책]
• "결국 종목은 안 알려주고 마인드셋만 강조"
→ 빈틈: 구체적 종목/시점 정보 부재
→ 기회: 실제 매매 사례 + 스크린샷
...
• "내용이 좋습니다" 같은 긍정 리뷰 분석 (의미 없음)
• "배송이 느렸어요" 같은 서비스 불만 (책 내용과 무관)
• 너무 일반적인 불만 ("어려워요", "재미없어요")

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📝 출력 형식
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

[매우 중요] 모든 답변은 반드시 한국어로만 작성하세요.

JSON:
{{
    "analysis_scope": {{
        "books_analyzed": "분석한 책 수",
        "reviews_analyzed": "분석한 리뷰 수",
        "negative_reviews": "부정 리뷰 수와 비율",
        "platforms": ["크몽", "예스24", "알라딘", "교보문고"]
    }},
    "negative_patterns": [
        {{
            "pattern": "가장 많은 불만 패턴 (구체적으로)",
            "frequency": "빈도 (%)",
            "example_reviews": ["실제 리뷰처럼 쓴 예시 (말투까지)", "예시2"],
            "reader_emotion": "이 리뷰 쓴 독자가 느낀 감정",
            "hidden_need": "이 불만 뒤에 숨겨진 진짜 니즈",
            "my_solution": "내 책에서 이걸 어떻게 해결할지"
        }}
    ],
    "market_gaps": {{
        "빈틈들": ["시장에서 채워지지 않은 니즈 1", "니즈 2", "니즈 3"],
        "가장_큰_기회": "가장 크게 공략할 수 있는 빈틈",
        "왜_기회인가": "이 빈틈이 왜 중요한지 2문장"
    }},
    "concept_suggestions": [
        {{
            "concept": "이 빈틈을 공략하는 컨셉",
            "hook": "이 컨셉의 마케팅 훅 한 문장",
            "why_works": "왜 이게 통할지",
            "differentiation": "기존 책들과 뭐가 다른지"
        }}
    ],
    "action_items": {{
        "must_include": ["내 책에 반드시 넣어야 할 것들"],
        "must_avoid": ["기존 책들이 욕먹은 것들 (절대 하면 안 됨)"],
        "unique_selling_point": "내 책만의 차별점 한 문장"
    }}
}}"""
    return ask_ai(prompt, 0.6)
...
def analyze_text_content(text, source=""):
    """콘텐츠 심층 분석 - 전자책에 활용할 인사이트 추출"""
    prompt = f"""출처: {source}
내용: {text[:5000]}

이 콘텐츠에서 전자책에 바로 쓸 수 있는 인사이트를 추출해주세요.
...
1. 반전 포인트
   - "대부분 A라고 생각하는데 실제로는 B"
   - 상식을 뒤집는 주장

2. 구체적 숫자/사례
   - "월 340만원", "47일 만에", "전환율 3.7%"
   - 실제 경험담, 성공/실패 사례
...
JSON:
{{
    "main_topic": "이 콘텐츠의 핵심 주제",
    "key_points": [
        "핵심 포인트 1 (구체적 숫자/사례 포함)",
        "핵심 포인트 2",
        "핵심 포인트 3",
        "핵심 포인트 4",
        "핵심 포인트 5"
    ...
    "actionable_tips": [
        "바로 실행 가능한 팁 1 (구체적 방법 포함)",
        "실행 팁 2",
        "실행 팁 3"
    ],
    "ebook_applications": [
        "내 전자책에서 이렇게 활용: 구체적 방법",
        "이 인사이트를 챕터에 녹이는 법"
    ],
    "summary": "이 콘텐츠의 핵심 3문장 요약"
}}"""
    return ask_ai(prompt, 0.5)
...
def generate_questions(subtopic, chapter, topic):
    """본문 작성을 위한 심층 질문 생성 - 자청/프드프 스타일"""
    prompt = f"""'{subtopic}' 본문을 쓰기 위한 핵심 질문 3개를 만들어주세요.
...
저자의 실제 경험과 인사이트를 끌어내서
"이 사람 진짜 해본 사람이네" 느끼게 하는 본문을 쓰기 위함
...
[구체적 숫자를 끌어내는 질문]
• "이 방법으로 첫 수익이 나기까지 며칠 걸렸나요?"
• "실패했을 때 구체적으로 얼마를 잃었나요?"
• "가장 효과 있었던 방법의 전환율은 몇 %였나요?"

[반전 스토리를 끌어내는 질문]
• "처음에 당연하다고 생각했는데 완전히 틀렸던 게 뭔가요?"
• "다들 A가 정답이라고 하는데, 왜 B가 맞다고 생각하나요?"
• "가장 큰 실수는 뭐였고, 그래서 뭘 깨달았나요?"

[실전 노하우를 끌어내는 질문]
• "초보자가 가장 먼저 해야 할 딱 한 가지는?"
• "이건 절대 하면 안 된다, 하는 게 있다면?"
• "남들은 모르는데 업계에서만 통하는 팁이 있나요?"
...
Q1: [구체적 숫자/경험을 끌어내는 질문]
Q2: [반전/깨달음 스토리를 끌어내는 질문]
Q3: [실전 팁/노하우를 끌어내는 질문]
...
[블로그 분야]
• 핫 키워드: "AI 블로그", "챗GPT 글쓰기", "자동화"
• 떠오르는 주제: "블로그 없이 수익내기", "숏폼 연계"
• 시장 빈틈: "AI 활용하면서도 상위노출 되는 법"

[투자 분야]
• 핫 키워드: "배당주", "미국 ETF", "금리 인하"
• 떠오르는 주제: "인플레이션 대응", "월세형 포트폴리오"
• 시장 빈틈: "직장인이 30분으로 관리하는 포트폴리오"

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
❌ 쓸모없는 분석 (금지)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

• "성공", "부자", "자기계발" 같은 너무 일반적인 키워드
• "꾸준히 하세요" 같은 뻔한 조언
• 실제 구매로 이어지지 않는 추상적 분석
...
JSON 형식으로 응답:
{{
    "hot_keywords": ["지금 검색량 높은 키워드 (구체적)", "키워드 2", "키워드 3", "키워드 4", "키워드 5"],
    "rising_topics": ["최근 3개월 급상승 주제 (왜 뜨는지 이유 포함)", "주제 2", "주제 3"],
    "reader_needs": ["독자가 진짜 원하는 것 (표면+숨겨진 니즈)", "니즈 2", "니즈 3"],
    "content_gaps": ["시장에서 아직 없는 콘텐츠 (기회)", "빈틈 2"],
    "recommended_angles": ["이 분야에서 통하는 접근법 (실제 베스트셀러 예시)", "접근법 2", "접근법 3"],
    "avoid": ["이미 포화된 주제 (왜 피해야 하는지)", "피할 것 2"],
    "winning_concept": "지금 이 시장에서 통할 컨셉 한 문장"
}}"""
                        result = ask_ai(prompt, 0.8)
                        parsed = parse_json(result)
                        if parsed:
        ...
        with col1:
            ...
            if st.button("경쟁서 분석", use_container_width=True, key="comp_btn"):
                if not comp_title or not comp_toc:
                    ...
                    with st.spinner("분석 중..."):
                        prompt = f"""다음 경쟁 도서를 분석해서 내 책의 차별화 포인트를 찾아주세요:
...
[강점 분석]
• "목차만으로 로드맵이 그려짐" → 구조화 잘 됨
• "실제 사례가 많음" → 신뢰도 높음

[약점 분석]
• "이론은 좋은데 실제로 뭘 해야 할지 모르겠음"
  → 기회: 단계별 실행 가이드 제공
• "너무 초보용이라 아는 내용 반복"
  → 기회: 중급자 타겟으로 차별화
...
JSON 형식으로 응답:
{{
    "book_summary": "이 책의 핵심 컨셉 한 문장 (무슨 약속을 하는 책인지)",
    "target_audience": "이 책의 타겟 (구체적으로: 나이, 상황, 고민)",
    "strengths": [
        "강점 1 (왜 이게 강점인지, 어떤 독자에게 어필하는지)",
        "강점 2",
        "강점 3"
    ],
    "weaknesses": [
        "약점 1 (리뷰에서 불만이 나올 만한 포인트)",
        "약점 2 (목차에서 보이는 한계점)"
    ],
    "unique_selling_point": "이 책이 팔리는 진짜 이유",
    "improvement_opportunities": [
        "내 책에서 이 약점을 어떻게 보완할지 (구체적 방법)",
        "이 책에 없는 걸 내가 제공할 수 있는 것",
        "이 책보다 내가 잘할 수 있는 영역"
    ],
    "marketing_angle": "이 책 대비 내 책을 어필할 한 문장",
    "content_structure": "목차 구성 방식 (참고할 점 + 개선할 점)"
}}"""
...
크몽/클래스101 썸네일에서 3초 안에 클릭하게 만드는 것
"이 책 뭔가 다르네, 전문가가 쓴 것 같네" 느끼게
...
JSON 형식으로 응답:
{{
    "recommended_style": "추천 스타일 (위 3가지 중 하나 + 이유)",
    "color_scheme": "구체적 색상 조합 (HEX 코드 포함, 예: #1a1a2e 배경 + #d4af37 포인트)",
    "design_concept": "이 주제에 맞는 디자인 컨셉 (왜 이 스타일이 어울리는지)",
    "typography_tip": "폰트 추천 (무료 한글 폰트명 + 크기 + 자간)",
    "layout_tip": "레이아웃 팁 (제목 위치, 여백, 정렬)",
    "reference_books": ["이 분야 베스트셀러 표지 1 (왜 참고할 만한지)", "표지 2", "표지 3"],
    "canva_search_keyword": "Canva에서 검색할 영문 키워드 (2-3개)",
    "avoid": "이 분야에서 피해야 할 디자인 요소"
}}"""
                    result = ask_ai(prompt, 0.7)
                    parsed = parse_json(result)
                    if parsed:
