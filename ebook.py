...
def init_browser_id():
    ...
    if COOKIE_AVAILABLE:
        try:
            ...
            # 기기 ID
            if cookies and 'writey_device_id' in cookies:
                st.session_state['browser_device_id'] = cookies['writey_device_id']

            # 비밀번호
            if cookies and 'writey_password' in cookies:
                st.session_state['saved_password'] = cookies['writey_password']

            # API 키
            if cookies and 'writey_api_key' in cookies:
...
def save_device_id_to_browser(device_id):
    ...
    if COOKIE_AVAILABLE:
        try:
            cookie_manager = get_cookie_manager()
            cookie_manager.set('writey_device_id', device_id, max_age=365*24*60*60)
        except:
            pass
...
def save_password_to_browser(password):
    ...
    if COOKIE_AVAILABLE:
        try:
            cookie_manager = get_cookie_manager()
            cookie_manager.set('writey_password', password, max_age=365*24*60*60)
        except:
            pass
...
def save_api_key_to_browser(api_key):
    ...
    if COOKIE_AVAILABLE:
        try:
            cookie_manager = get_cookie_manager()
            cookie_manager.set('writey_api_key', api_key, max_age=365*24*60*60)
        except:
            pass
