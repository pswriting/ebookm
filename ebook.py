...
def init_browser_id():
    """브라우저 ID 초기화 - localStorage에서 불러오기"""
    # 이미 초기화되었으면 스킵
    if 'browser_device_id' in st.session_state and st.session_state['browser_device_id']:
        return

    # streamlit-javascript로 localStorage에서 기기 코드 불러오기
...
def get_saved_password():
    """브라우저 localStorage에서 저장된 비밀번호 불러오기"""
    if BROWSER_ID_AVAILABLE:
        try:
...
def get_saved_api_key():
    """브라우저 localStorage에서 저장된 API 키 불러오기"""
    if BROWSER_ID_AVAILABLE:
        try:
            saved_key = st_javascript("localStorage.getItem('writey_api_key')")
            if saved_key and saved_key != 0 and saved_key != "null":
                return saved_key
        except:
            pass
    return None
