...
def estimate_docx_pages(chapters, outline):
    ...
    """
    import math
    CPP = 600  # A5 10.5pt/1.85 기준 한 페이지 본문 글자수(공백 제외) 근사
    FRONT = 2  # 표지 + 판권
    PROLOGUE = 2
    EPILOGUE = 2
    AUTHOR = 1
...
def create_ebook_docx(title, subtitle, author, chapters_data, outline, interview_data=None):
    ...
    try:
        ...
        def body_paragraph(text, first=False, size=10.5, ls=1.75):
            p = doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            set_font(p.add_run(text), size, serif=True, color=_INK)
            pf = p.paragraph_format
            pf.line_spacing = ls; pf.space_after = Pt(12)
            if not first:
                pf.first_line_indent = Cm(0.5)
            return p

        def drop_cap(letter, rest, lines=3, size=10.5, ls=1.75):
            dp = doc.add_paragraph()
            pPr = dp._p.get_or_add_pPr()
            frame = OxmlElement('w:framePr')
            frame.set(qn('w:dropCap'), 'drop')
            frame.set(qn('w:lines'), str(lines))
            frame.set(qn('w:wrap'), 'around')
            frame.set(qn('w:vAnchor'), 'text')
            frame.set(qn('w:hAnchor'), 'text')
            pPr.insert(0, frame)
            set_font(dp.add_run(letter), size * lines * 0.92, serif=True, color=_INK)
            bp = doc.add_paragraph()
            bp.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
            set_font(bp.add_run(rest), size, serif=True, color=_INK)
        ...
        if BRAND:
            ...
            set_font(bp.add_run(track_text(BRAND, 2)), 8.5, serif=False, color=_FAINT)
            bp.paragraph_format.space_after = Pt(26)
        tp = doc.add_paragraph(); tp.alignment = WD_ALIGN_PARAGRAPH.CENTER
        set_font(tp.add_run(title), 30, bold=True, color=_INK, serif=True)
        tp.paragraph_format.space_after = Pt(18); tp.paragraph_format.line_spacing = 1.2
        hairline(width_cm=2.2, color=_GOLD, space_before=4, space_after=18, size=8)
        if subtitle:
            sp = doc.add_paragraph(); sp.alignment = WD_ALIGN_PARAGRAPH.CENTER
            set_font(sp.add_run(subtitle), 12, italic=True, color=_SOFT, serif=True)
        vspace(120)
        ap = doc.add_paragraph(); ap.alignment = WD_ALIGN_PARAGRAPH.CENTER
        set_font(ap.add_run(track_text(author or "저자", 1)), 12, serif=False, color=_INK)
        ap2 = doc.add_paragraph(); ap2.alignment = WD_ALIGN_PARAGRAPH.CENTER
        set_font(ap2.add_run("지음"), 9.5, serif=False, color=_FAINT)
        ap2.paragraph_format.space_before = Pt(4)
        doc.add_page_break()
...
본문 텍스트만 출력하세요. 어떤 마크다운 헤더(#)나 '프롤로그' 라벨도 출력하지 마세요. 첫 문장부터 바로 본문이 시작되어야 합니다."""
        ...
        set_font(ct.add_run(track_text("CONTENTS", 3)), 13, serif=False, color=_INK)
        ct.paragraph_format.space_after = Pt(10)
        hairline(width_cm=1.4, color=_GOLD, space_after=34, size=8)
        for idx, chapter in enumerate(outline):
            ...
            row.paragraph_format.space_before = Pt(20); row.paragraph_format.space_after = Pt(4)
            set_font(row.add_run(f"{idx+1:02d}"), 13, serif=False, color=_GOLD)
            set_font(row.add_run("   "), 13, serif=False, color=_GOLD)
            add_hyperlink(row, clean, f"chapter_{idx+1}", size=11.5, bold=True, color=_INK, serif=True)
            if chapter in chapters_data:
                for sub in chapters_data[chapter].get('subtopics', []):
                    sr = doc.add_paragraph()
                    sr.paragraph_format.left_indent = Cm(0.9); sr.paragraph_format.space_after = Pt(3)
                    si = chapters_data[chapter]['subtopics'].index(sub)
                    add_hyperlink(sr, sub, f"subtopic_{idx+1}_{si+1}", size=9.5, bold=False, color=_SOFT, serif=True)
        doc.add_page_break()
        ...
        for idx, chapter in enumerate(outline):
            ...
            set_font(lab.add_run(track_text("PART", 2)), 9, serif=False, color=_FAINT)
            lab.paragraph_format.space_after = Pt(6)
            num = doc.add_paragraph(); num.alignment = WD_ALIGN_PARAGRAPH.CENTER
            set_font(num.add_run(f"{idx+1:02d}"), 56, serif=True, color=(120, 120, 126))
            num.paragraph_format.space_after = Pt(10)
            hairline(width_cm=1.6, color=_GOLD, space_after=22, size=8)
            cn = doc.add_paragraph(); cn.alignment = WD_ALIGN_PARAGRAPH.CENTER
            add_bookmark(cn, f"chapter_{idx+1}")
            set_font(cn.add_run(clean), 17, bold=True, color=_INK, serif=True)
            cn.paragraph_format.line_spacing = 1.25; cn.paragraph_format.space_after = Pt(40)
            ...
            for si, sub in enumerate(subs):
                ...
                hn.paragraph_format.space_before = Pt(6); hn.paragraph_format.space_after = Pt(2)
                set_font(hn.add_run(f"{idx+1}.{si+1}"), 9.5, serif=False, color=_GOLD, track=0.5)
                ht = doc.add_paragraph()
                add_bookmark(ht, f"subtopic_{idx+1}_{si+1}")
                set_font(ht.add_run(sub), 14, bold=True, color=_INK, serif=True)
                ht.paragraph_format.space_after = Pt(4); ht.paragraph_format.line_spacing = 1.2
                hairline(align=WD_ALIGN_PARAGRAPH.LEFT, color=_HAIR, space_after=18, size=4)
...
def _fallback_method_name(topic):
    import random
    head = random.choice([
        "Compound", "Cashflow", "Dividend", "Yield", "Anchor", "Core",
        "Prime", "Orbit", "Relay", "Forge", "Pivot", "Summit", "Ledger",
        "Compass", "Atlas", "Vault", "Beacon", "Bedrock", "Catalyst", "Momentum"
    ])
    tail = random.choice(["Loop", "Stack", "Ladder", "Engine", "Method",
                          "System", "Framework", "Protocol", "Model", "Blueprint"])
    return f"{head} {tail}"


def get_or_create_method_name(topic, interview_data=None, force_new=False):
    """이 책의 기법 이름(영문)을 만들거나 가져온다.

    - force_new=True: 새 전자책 → 이전 책들과 겹치지 않는 새 이름 생성
...
규칙:
- 반드시 영어로만. 한글·한자 절대 금지.
  좋은 예: "Cashflow Ladder", "Dividend Engine", "Compound Loop", "Yield Stack", "DIP Method"
- 2~3단어, 또는 (영문 약자 + 영어 단어). 발음 가능하고 의미가 통할 것.
- 주제와 직관적으로 연결될 것. 의미 없는 멋부린 조합 금지.
- 아래 '이미 쓴 이름'들과 철자·의미 모두 겹치지 말 것:
  {used_str}
- 다양성 시드 {seed} 를 참고해 매번 다른 결과를 낼 것.

JSON만 출력:
{{"method_name": "영문 기법 이름"}}"""
    ...
    try:
        ...
        if parsed:
            name = (parsed.get('method_name') or '').strip()
    except Exception:
        name = None

    def _valid(n):
        # 영문/숫자/공백/.- 만 허용 (한글이 섞이면 탈락) + 길이 + 중복 아님
        return bool(n) and re.fullmatch(r"[A-Za-z][A-Za-z0-9 .\-]{1,28}", n or "") is not None and n not in used

    if not _valid(name):
        name = _fallback_method_name(topic)
        tries = 0
        while name in used and tries < 12:
            name = _fallback_method_name(topic)
            tries += 1
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
이 책의 기법 이름은 정확히 "{name}" (영문) 으로 이미 확정되어 있다.
- 모든 곳에서 이 이름을 토씨 하나 바꾸지 말고 그대로 사용한다.
- 한글로 번역/의역하거나, 새 이름을 짓거나, 다른 표기로 바꾸는 것을 절대 금지한다.
- ○○○ / [컨셉명] / [시스템명] 자리에는 전부 "{name}" 을 넣는다.
- "{name}" 뒤에 시스템·공식·구조·법칙 같은 한글 접미사를 붙이지 않는다. 이름 그대로 쓴다.
- 이 이름은 아래 등장하는 어떤 '금지어 목록(파이프라인/엔진 등)'에도 해당되지 않는다. 그대로 사용한다.
"""
...
이 책의 기법 이름은 이미 "{method_name}" (영문) 으로 확정되어 있습니다.
...
이 책의 기법 이름은 "{method_name}" (영문)으로 확정됨. 제목에 기법 이름을 넣을 경우 반드시 이 영문 그대로 사용(한글 번역/재작명 금지).
...
기법 이름은 위에서 "{method_name}" (영문)으로 이미 확정되었다.
- 새로 작명하지 말고, 한글로 바꾸지 말고, 그대로 쓴다.
- 아래 예시의 ○○○ 자리에는 전부 "{method_name}" 을 넣는다.
- "{method_name}" 은 그 자체로 쓴다. 뒤에 시스템/공식/구조/법칙을 붙이지 않는다.
...
이 책의 기법 이름은 "{st.session_state.get('method_name', '')}" (영문)으로 확정됨. 본문에서 기법을 언급할 때 반드시 이 영문 그대로 사용(한글 번역/재작명 금지).
...
기법 이름은 위에서 "{method_name}" (영문)으로 이미 확정됨. 새로 짓지 말 것.
...
if header_video_b64:
    ...
    <div style="position:relative;border-radius:16px;overflow:hidden;margin-bottom:35px;border:0.5px solid rgba(201,162,75,0.18);">
        ...
        <div style="position:absolute;top:0;left:0;right:0;bottom:0;display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;">
            <div class="writey-eyebrow">AI EBOOK Writer</div>
