...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
★★★ 최우선 코어 — 이 블록이 이 프롬프트의 모든 규칙에 절대 우선한다 ★★★
(아래에 이와 다른 지시·예시·자가점검이 있어도, 충돌하면 무조건 이 코어를 따른다. 특히 '짧게', '반복 금지', '근거(지렛대) 노출'은 이 코어가 최종 결정이다.)

[목표] 서점에서 5초 훑은 사람이 "이건 사야 해"라고 결제하게 만드는 목차. 설명은 본문이 하고, 목차는 100% 구매욕만 자극한다.

[핵심 5원칙]
1) 스토리 흐름 — PART 1~5 제목을 세로로 이어 읽으면 한 사람의 변화가 '한 편의 이야기'로 보인다. 바닥(지금 너의 현실) → 통념 박살(네 탓이 아니었다) → 첫 사건(처음 바뀐 순간) → 전환(이제 다른 사람) → 도약(완전히 다른 인생). PART 1은 가장 아프게, PART 5는 가장 벅차게. 두 PART가 같은 말이면 실패.
2) 짧고 직관적 — 한 줄에 한 가지만. 12~22자, 한 호흡에 읽히게, 3초 안에 이해되게. 비교는 한 쌍만(8%·4%와 5년·2년 동시 금지), 정보(숫자·기간·원리)는 하나만, 수식어 겹치기("똑같았던 마지막 석 달") 금지.
   ★ 주어·목적어를 생략해 모호하게 만들지 마라 — 무엇을 받고/벌고/쌓는지 반드시 드러내라. ("비는 달 없이 받는 법"→"비는 달 없이 배당금 받는 법", "더 쌓았다"→"자산을 더 쌓았다", "더 벌었다"→"돈을 더 벌었다")
3) 주장 말고 근거 — 빈 선언("달라진다/완성된다/무너지지 않는다") 금지. 지렛대(행동·원리)를 한 조각 보여 "이렇게 하니 되는구나" 믿게 하라. 단 실행 디테일(정확한 종목·비율·타이밍)은 숨겨 본문으로 남긴다. 강한 공식: "[지렛대]로 [결과]가 일어난 [까닭/순간]".
4) 반복 금지 — 25줄에서 핵심 단어·문장 끝 패턴·숫자는 딱 한 번만(조사·기법명·핵심 주제어는 예외). '이유·차이·결정적·신호·처음·가장 먼저'가 두 줄에 있으면 한쪽을 다른 말로.
5) 자연스러운 한국어 — 원어민 편집자가 손 안 대고 통과시킬 문장만. 추상·돈·계좌에 은유 동사(갉아먹다·뒤집히다·굴러가다·몸집을 불리다) 붙이지 마라. 은유 대신 구체 장면·숫자로 직진. AI 클리셰(졸업·정체·진짜 게임·결정적 시그널) 금지.
6) 차별화 + 자극(자청 톤) — '그냥 또 다른 전자책'처럼 보이면 실패다. 이 책만의 '한 문장 역발상 주장'(남들은 안 하는 통념 뒤집기)을 정하고, 그게 목차 전체를 관통하는 중심축이 되게 하라. 기법명 "{method_name}"이 바로 그 축의 이름이다.
   · PART 1~2는 특히 도발적으로 — 통념을 정면으로 부수고(예: "고배당을 좇을수록 더 가난해진다", "의지가 강할수록 더 크게 무너진다"), 독자의 자기기만을 콕 찌른다(예: "당신이 못 모은 건 의지가 약해서가 아니다").
   · 가끔 명령조로 단언하라(예: "종목을 고르지 마라, 날짜를 설계하라"). 단정적이고 셀수록 더 끌린다.
   · 단, 도발은 '통념·행동'을 때리되 독자를 모욕하진 마라. 어법 자연스러움은 언제나 우선.

[★★ 뇌과학 자극 엔진 — '미치도록 사고 싶게' 만드는 핵심. 모든 줄에 최소 1개, 센 줄엔 2개를 겹쳐라]
뇌는 (1) 예측이 깨질 때 (2) 빈칸을 보면 채우려 할 때 (3) 잃을 게 보일 때 도파민이 터져 "더 알고 싶다 / 안 하면 불안하다"는 충동을 만든다. 25줄 전부를 이 충동 위에 세워라. 정보를 '전달'하지 말고 충동을 '유발'하라.
① 손실 회피 — "이걸 모르면 5년을 또 날린다", "지금 안 바꾸면 내년에도 똑같은 자리"
② 통념 박살(예측 오류) — "열심히 할수록 더 가난해진다", "참는 사람이 가장 먼저 무너진다"
③ 호기심 갭 — 결과·장면은 보이고 이유·방법은 숨겨 본문을 사야 풀리게("왜/무엇이"로 끝맺기)
④ 충격 숫자·구체 장면 — "10명 중 9명", "47만원", "사흘이면" (막연한 '많은 사람' 금지)
⑤ 정체성 전환 — "이걸 익힌 사람은 다시 그 자리로 안 돌아간다"
⑥ 인그룹 사회증명 — "상위 3%만 아는", "먼저 시작한 사람들의 5년 뒤"
→ 가장 센 줄은 둘 이상 겹친다. 예: "10명 중 9명이(사회증명) 배당주를 사고도 매달 한 푼도 못 받는(손실+숫자) 이유(호기심 갭)". 밍밍한 줄은 죽은 줄 — 반드시 위 트리거로 다시 써라.

[기법 이름] "{method_name}"을 글자 그대로 쓴다. 소제목에만 정확히 3번, PART 제목엔 0번. 글자수·단어수 단정("세 글자") 금지. 약자 풀이({method_expansion})가 있으면 각 단계가 PART 흐름에 자연스럽게 드러나게(사전식 나열 금지).

[본보기 — 이 '길이와 결'을 그대로 복제하라. 문장은 베끼지 말고 [주제]에 맞춰 같은 길이로 새로 써라]
《예시 A · 월배당 투자》  ← 중심축: "배당률 말고 '들어오는 날짜'를 설계한다"
PART 1. 고배당을 좇을수록 더 가난해진다
- 매달 배당은 들어오는데 통장은 그대로인 이유
- 배당률 높은 종목일수록 원금을 잃기 쉬운 까닭
- "고배당이 답"이라는 믿음이 5년을 날린다
- 결국 부자는 배당률이 아니라 '○○○'를 본다
PART 2. 종목을 고르지 마라, 날짜를 설계하라
- 수익률보다 '언제 들어오느냐'가 통장을 바꾼다
- ETF 네 개로 비는 달 없이 배당금 받는 구조
- 고배당주를 피한 사람이 오히려 돈을 더 번다
- 폭락에 팔지 않고 더 산 사람이 자산을 먼저 회복한다
PART 3. 첫 배당 47만원이 통장에 찍힌 날
- 월급 아닌 돈이 처음 들어온 그 아침
- ○○○를 적용한 사람이 가장 먼저 바꾼 한 가지
- 이 석 달을 놓치면 1년을 다시 기다린다
- 한 번 현금흐름을 맛본 사람은 원금에 만족 못 한다
PART 4. 자는 동안에도 배당이 들어오기 시작한다
- 배당을 다시 산 사람만 받는 돈이 매년 커진다
- 월 백만원을 넘긴 뒤 가장 먼저 끊은 습관
- 계좌를 덜 본 해에 오히려 수익률이 더 높았다
- 폭락장에도 흔들리지 않는 포트폴리오의 공통점
PART 5. 월급을 넘어선 다음, 시간까지 번다
- 배당을 계속 산 사람과 빼 쓴 사람의 5년 뒤 자산 차이
- 현금흐름과 자산이 같이 불어나는 지점
- 회사를 그만둔 사람이 마지막 달에 똑같이 한 일
- ○○○로 자리잡은 사람은 다시 종목 검색으로 안 돌아간다

《예시 B · 멘탈 관리》  ← 중심축: "감정은 의지로 못 잡는다, 90초 타이밍으로 잡는다"
PART 1. 의지가 강한 사람일수록 더 크게 무너진다
- 마음을 백 번 다잡아도 사흘이면 무너지는 이유
- 참고 버틴 사람의 멘탈이 가장 먼저 박살 난다
- "긍정적으로 생각하라"가 당신을 더 망친다
- 멘탈은 의지가 아니라 '○○○'로 잡는다
PART 2. 감정은 누르지 마라, 90초를 기다려라
- 감정이 터질 때 뇌가 가장 먼저 멈추는 지점
- 참을수록 더 크게 터지는 사람들의 공통점
- 명상도 운동도 당신에게만 안 통한 진짜 까닭
- 무너진 다음 날 가장 먼저 해야 할 한 가지
PART 3. 폭발 직후 90초가 회복의 8할을 가른다
- 그 90초를 놓치면 며칠을 다시 잃는다
- ○○○를 처음 써본 사람이 느낀 변화
- 단번에 가라앉히는 사람이 무심코 하는 행동
- 처음으로 감정에 안 휘둘린 그날
PART 4. 한 번 회복한 뇌는 같은 일에 안 흔들린다
- 같은 일을 겪어도 더는 휘둘리지 않게 되는 원리
- 무너지는 횟수가 한 달 만에 절반으로 준다
- 단단해진 사람이 더는 하지 않는 행동 세 가지
- 주변에서 "사람이 달라졌다"는 말을 듣는 시점
PART 5. 단단한 멘탈이 인생 전체를 바꾼다
- 멘탈이 바뀌자 통장과 인간관계가 같이 달라졌다
- 다시는 예전으로 안 돌아가는 사람들의 차이
- 반년 뒤 가장 크게 달라지는 한 가지
- ○○○로 회복한 사람의 5년 뒤

[출력 형식] PART 1.부터 PART 5.까지, 각 PART 아래 '- 소제목' 정확히 4개. 예시의 ○○○ 자리엔 "{method_name}"을 넣되 소제목에만 3번. 콜론(:) 금지. 목차만 출력하고 점검·설명은 출력하지 마라.
[어법은 처음부터 완벽하게] 별도 교정 단계는 없다. 출력하기 전에 모든 줄을 스스로 소리내어 읽어, 원어민이 한 번에 이해되고 어법이 자연스러운지 확인하고, 어색하면 자극을 유지한 채 고쳐서 내보내라.

[출력 전 점검] ① 세로로 이어 읽으면 한 이야기로 읽히고, 같은 말 하는 PART가 없나 ② 모든 줄이 짧고 3초 안에 직관적으로 이해되나 ③ 빈 선언이 아니라 지렛대·원리가 보이나(디테일은 숨김) ④ 핵심 단어·문장 끝·숫자가 반복되지 않나 ⑤ 은유 오용·정보 욱여넣기로 비문이 된 줄이 없나 ⑥ 기법명이 소제목에 3번·PART 제목에 0번인가 ⑦ 5초 훑은 독자가 '미치도록 사고 싶다'고 느낄 만큼, 모든 줄이 손실회피·통념박살·호기심갭 중 하나 이상을 건드리는가? 한 줄이라도 밍밍하면 더 세게 다시 써라.

═══ 여기까지가 코어다. 아래 내용은 참고용 보조 설명이며, 위 코어와 조금이라도 충돌하면 전부 무시한다. ═══
...
⚠️ 위는 '톤·구조의 본보기'다. 문장을 그대로 베끼지 말고, 주어진 [주제]·[저자 노하우]에 맞춰 같은 강도로 새로 써라.
...
✓ "배당을 안 쓰고 다시 산 사람만 원금이 불어난 이유" (지렛대=재투자, 원리=복리 / 금액·종목은 숨김)
✓ "폭락에 팔지 않고 더 산 사람이 먼저 회복한다" (지렛대=급락 매수, 인과가 보임 / 비중·시점은 숨김)
✓ "월급과 배당을 분리한 사람만 돈이 따로 쌓인다" (구조의 논리가 보임)
✓ "3년간 빼지 않고 재투자만 했더니 원금이 두 배가 됐다" (지렛대=재투자 누적 / 결과의 근거가 보임)
...
[자청식 후킹 7가지 무기 — 소제목 20개 중 최소 14개에 적용 (밍밍한 정보형은 거의 없어야 함)]
...
[하드 규칙 — 어기면 즉시 다시 써라]
· 한 줄은 한 가지만 말한다. 정보(비교·숫자·기간·원리)는 한 줄에 '하나'만 넣는다.
· 비교는 한 줄에 한 쌍만. "연 8% vs 월배당 4%"와 "5년 vs 2년"을 한 줄에 같이 넣지 마라 — 둘 중 더 센 하나만 남긴다. ("8% 고배당을 좇던 5년보다 월배당 4%로 쌓은 2년이 더 많이 남은 까닭" → ✗ 너무 복잡 / ✓ "고배당만 좇다 2년을 날린 사람이 뒤늦게 깨달은 것")
· 수식어(꾸미는 말)를 겹치지 마라. "똑같았던 마지막 석 달"(똑같았던+마지막)처럼 관형어를 포개면 어색하다.
· 길이는 한 호흡(대략 12~22자). 읽다가 숨이 차거나 쉼표로 절을 잇고 있으면 자르거나 둘로 쪼개라.
· 완성 후 각 줄을 '3초 안에 직관적으로' 이해 못 하면 실패다 — 더 짧고 쉽게 고쳐라. 짧을수록 더 세고 더 끌린다.
...
목차만 출력. 콜론 금지. 매 소제목 다른 패턴. 명사·동사 엔딩 섞기. 같은 단어·표현·문장 끝은 전체에서 한 번만. 5개 PART는 이어 읽으면 서로 다른 5단계의 한 이야기. 어법 어색한 표현 즉시 폐기. 각 PART는 정확히 소제목 4개. 컨셉명 글자수 단정 금지. 자가점검 결과는 출력하지 말 것."""

        chapters = []
        subtopics = {}
        for _outline_attempt in range(2):
            outline_result = ask_ai(outline_prompt, 0.95, ensure_quality=True)
            if not outline_result:
                continue

            # [어법 2차 교정 패스 제거] 별도 교정이 표현을 약화시키는 부작용이 있어 제거.
            # 대신 생성 단계(코어 프롬프트)에서 처음부터 어법·문법을 맞춰 쓰고 결과를 그대로 사용한다.
...
목차만 출력. 콜론 금지. 매 소제목 다른 패턴. 명사·동사 엔딩 섞기. 같은 단어·표현·문장 끝은 전체에서 한 번만. 5개 PART는 이어 읽으면 서로 다른 5단계의 한 이야기. 어법 어색한 표현 즉시 폐기. 각 PART는 정확히 소제목 4개. 컨셉명 글자수 단정 금지. 자가점검 결과는 출력하지 말 것."""
    _raw = ask_ai(prompt, 0.95, ensure_quality=True)
    # 어법 전담 재검토 — 어색한 표현을 자연스럽게 교정
    # [어법 2차 교정 패스 제거] 표현 약화 방지를 위해 생성 결과를 그대로 반환.
    return _raw
