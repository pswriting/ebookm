...
[출력 전 점검] ① 세로로 이어 읽으면 한 이야기로 읽히고, 같은 말 하는 PART가 없나 ② 모든 줄이 짧고 3초 안에 직관적으로 이해되나 ③ 빈 선언이 아니라 지렛대·원리가 보이나(디테일은 숨김) ④ 핵심 단어·문장 끝·숫자가 반복되지 않나 ⑤ 은유 오용·정보 욱여넣기로 비문이 된 줄이 없나 ⑥ 기법명이 소제목에 3번·PART 제목에 0번인가.
...
목차만 출력. 콜론 금지. 매 소제목 다른 패턴. 명사·동사 엔딩 섞기. 같은 단어·표현·문장 끝은 전체에서 한 번만. 5개 PART는 이어 읽으면 서로 다른 5단계의 한 이야기. 어법 어색한 표현 즉시 폐기. 각 PART는 정확히 소제목 4개. 컨셉명 글자수 단정 금지. 자가점검 결과는 출력하지 말 것."""
        ...
        for _outline_attempt in range(2):
            ...
            # 어법 전담 재검토 — 어색한 표현을 자연스럽게 교정한 뒤 파싱
            try:
                progress_placeholder.info("✍️ 3/4 목차 어법 검토 중...")
            except Exception:
                pass
            outline_result = review_outline_grammar(outline_result, method_name)
...
목차만 출력. 콜론 금지. 매 소제목 다른 패턴. 명사·동사 엔딩 섞기. 같은 단어·표현·문장 끝은 전체에서 한 번만. 5개 PART는 이어 읽으면 서로 다른 5단계의 한 이야기. 어법 어색한 표현 즉시 폐기. 각 PART는 정확히 소제목 4개. 컨셉명 글자수 단정 금지. 자가점검 결과는 출력하지 말 것."""
    _raw = ask_ai(prompt, 0.92, ensure_quality=True)
    # 어법 전담 재검토 — 어색한 표현을 자연스럽게 교정
    return review_outline_grammar(_raw, method_name)
