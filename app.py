...
# 메인 UI
# ==========================================
# 비디오 배경 헤더 — title_bg.mp4 를 여러 위치에서 자동 탐색
def _find_header_video():
    candidates = []
    try:
        here = os.path.dirname(os.path.abspath(__file__))
        candidates.append(os.path.join(here, "title_bg.mp4"))
    except Exception:
        pass
    candidates += [
        "title_bg.mp4",
        os.path.join(os.getcwd(), "title_bg.mp4"),
        os.path.expanduser("~/Desktop/title_bg.mp4"),
        "/Users/hyunwoo/Desktop/title_bg.mp4",
    ]
    for p in candidates:
...
if header_video_b64:
    ...
    <div style="position:relative;border-radius:16px;overflow:hidden;margin-bottom:35px;border:0.5px solid rgba(201,162,75,0.25);box-shadow:0 18px 50px rgba(0,0,0,0.45);">
        <video autoplay muted loop playsinline style="width:100%;height:320px;object-fit:cover;filter:brightness(0.45) saturate(1.15) contrast(1.05);">
            <source src="data:video/mp4;base64,{header_video_b64}" type="video/mp4">
        </video>
        <div style="position:absolute;top:0;left:0;right:0;bottom:0;background:radial-gradient(ellipse at center, rgba(11,11,13,0.25) 0%, rgba(11,11,13,0.72) 100%);"></div>
