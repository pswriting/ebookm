...
def create_ebook_docx(title, subtitle, author, chapters_data, outline, interview_data=None):
    ...
    try:
        ...
        # 상단 골드 더블 라인
        vspace(60)
        hairline(width_cm=11.0, color=_GOLD, space_before=0, space_after=3, size=10)
        hairline(width_cm=11.0, color=_GOLD, space_before=0, space_after=4, size=4)
        if BRAND:
            bp = doc.add_paragraph(); bp.alignment = WD_ALIGN_PARAGRAPH.CENTER
            set_font(bp.add_run(track_text(BRAND, 3)), 9, serif=False, color=_GOLD)
            bp.paragraph_format.space_before = Pt(8); bp.paragraph_format.space_after = Pt(2)
        # 작은 장식
        em0 = doc.add_paragraph(); em0.alignment = WD_ALIGN_PARAGRAPH.CENTER
        set_font(em0.add_run("◆"), 8, serif=False, color=_FAINT)
        em0.paragraph_format.space_after = Pt(4)

        vspace(70)

        # 제목 — 다크 밴드(풀 너비) 위에 큰 세리프
        band_top = doc.add_paragraph(); band_top.alignment = WD_ALIGN_PARAGRAPH.CENTER
        band_top.paragraph_format.space_before = Pt(0); band_top.paragraph_format.space_after = Pt(0)
        band_top.paragraph_format.line_spacing = Pt(6)
        _shade_para(band_top, '1C1A17')
        for ln_i, line in enumerate(_wrap_title_lines(title, max_chars=9, max_lines=3)):
            ...
            _shade_para(tp, '1C1A17')
            set_font(tp.add_run(line), 32, bold=True, color=(245, 241, 232), serif=True)
        band_bot = doc.add_paragraph(); band_bot.alignment = WD_ALIGN_PARAGRAPH.CENTER
        band_bot.paragraph_format.space_before = Pt(0); band_bot.paragraph_format.space_after = Pt(0)
        band_bot.paragraph_format.line_spacing = Pt(6)
        _shade_para(band_bot, '1C1A17')

        if subtitle:
            sp = doc.add_paragraph(); sp.alignment = WD_ALIGN_PARAGRAPH.CENTER
            set_font(sp.add_run(subtitle), 12.5, italic=True, color=_SOFT, serif=True)
            sp.paragraph_format.space_before = Pt(20); sp.paragraph_format.line_spacing = 1.4

        vspace(120)
        # 저자 — 골드 라인 사이
        hairline(width_cm=2.0, color=_GOLD, space_before=0, space_after=12, size=6)
        ap = doc.add_paragraph(); ap.alignment = WD_ALIGN_PARAGRAPH.CENTER
        set_font(ap.add_run(track_text(author or "저자", 1)), 13, serif=False, color=_INK)
        ap2 = doc.add_paragraph(); ap2.alignment = WD_ALIGN_PARAGRAPH.CENTER
        set_font(ap2.add_run("지음"), 9.5, serif=False, color=_FAINT)
        ap2.paragraph_format.space_before = Pt(4); ap2.paragraph_format.space_after = Pt(12)
        hairline(width_cm=2.0, color=_GOLD, space_before=0, space_after=0, size=6)
        doc.add_page_break()
...
본문 텍스트만 출력하세요. 어떤 마크다운 헤더(#)나 '프롤로그' 라벨도 출력하지 마세요. 첫 문장부터 바로 본문이 시작되어야 합니다."""
        ...
        set_font(ct.add_run(track_text("CONTENTS", 3)), 15, serif=True, bold=True, color=_INK)
        ct.paragraph_format.space_after = Pt(4)
        cts = doc.add_paragraph(); cts.alignment = WD_ALIGN_PARAGRAPH.CENTER
        set_font(cts.add_run("차 례"), 10, serif=False, color=_FAINT, track=2)
        cts.paragraph_format.space_after = Pt(10)
        hairline(width_cm=1.4, color=_GOLD, space_after=30, size=8)
        ...
        for idx, chapter in enumerate(outline):
            ...
            # 챕터 줄: 큰 골드 번호 + 굵은 제목, 위에 가는 구분선
            if idx > 0:
                hairline(align=WD_ALIGN_PARAGRAPH.LEFT, color=(228, 224, 214),
                         space_before=8, space_after=10, size=4)
            head = doc.add_paragraph()
            head.paragraph_format.space_before = Pt(6); head.paragraph_format.space_after = Pt(2)
            set_font(head.add_run(f"PART {idx+1:02d}"), 10, serif=False, color=_GOLD, track=1.5)
            ctitle = doc.add_paragraph()
            ctitle.paragraph_format.space_after = Pt(8)
            add_hyperlink(ctitle, clean, f"chapter_{idx+1}", size=13.5, bold=True, color=_INK, serif=True)
            # 소제목: 점선 리더로 연결 (오른쪽 정렬 탭)
            if chapter in chapters_data:
                for sub in chapters_data[chapter].get('subtopics', []):
                    ...
                    sr.paragraph_format.left_indent = Cm(0.7); sr.paragraph_format.space_after = Pt(5)
                    sr.paragraph_format.line_spacing = 1.2
                    # 점선 리더 탭(오른쪽 끝)
                    sr.paragraph_format.tab_stops.add_tab_stop(
                        Cm(10.6), WD_TAB_ALIGNMENT.RIGHT, WD_TAB_LEADER.DOTS)
                    set_font(sr.add_run(f"{idx+1}.{si+1}  "), 9.5, serif=False, color=_GOLD)
                    add_hyperlink(sr, sub, f"subtopic_{idx+1}_{si+1}", size=10.5, bold=False, color=(70, 70, 76), serif=True)
        doc.add_page_break()
...
[호기심 강도 — 평이하면 결제 안 한다 (매우 중요)]
지금 가장 흔한 실패는 "평이함"이다. 아래처럼 결과·장면·숫자는 드러내고 원리·순서·정체는 숨겨, "본문을 사야 알 수 있게" 만들어라.
❌ 평이 → ✅ 호기심
- "배당주 고르는 법" → "10년 배당을 받고도 정작 한 푼도 못 쓰는 사람들의 공통점"
- "복리의 중요성" → "같은 돈을 넣었는데 7년 뒤 잔고가 두 배로 갈린 이유"
- "감정 관리가 필요하다" → "수익률 화면을 끄는 순간 오히려 계좌가 살아났다"
- "분산 투자를 하자" → "한 종목에 몰빵한 사람이 그해 오히려 덜 잃은 까닭"
- "꾸준함이 답이다" → "3년을 버틴 사람과 3개월에 그만둔 사람의 통장이 비슷한 이유"

규칙:
- 소제목 5개 중 3개 이상에 구체 숫자(금액·기간·비율)를 박는다.
- 통념을 정면으로 뒤집는 줄(예: "노력할수록 가난해진다")을 PART마다 최소 1개.
- 답을 같은 줄에서 다 말하지 마라. "왜/어떻게/무엇이"의 답은 본문에 숨긴다.
- 단, 호기심을 위해 억지 비유나 말 안 되는 단어 조합을 만들면 즉시 폐기. (아래 자연스러움 원칙 우선)

[자극 강도 — 자청 스타일로 끌어올려라 (매우 중요)]
밋밋한 정보형 제목은 전부 버린다. 통념을 정면으로 부수고, 독자의 자기합리화를 콕 찌르고, 단정적으로 선언하라.
- 통념 박살: "열심히 할수록 가난해지는 이유", "노력은 자주 배신한다"
- 자기기만 적발: "안 되는 걸 환경 탓으로 돌리는 사람의 5년 뒤", "공부만 하다 1년을 통째로 날리는 진짜 이유"
- 단정 선언: "결국 이거 하나에서 갈린다", "답은 처음부터 정해져 있었다"
- 날 선 대비: "버는 사람과 버는 척하는 사람", "시작한 사람과 준비만 한 사람의 통장"
- 숫자로 후려치기: "상위 3%만 알고 있는 한 가지", "10명 중 8명이 끝내 놓치는 것"
원칙:
- 사람을 모욕하지 마라(욕설·인신공격·특정인 비하 금지). 통념과 '행동'을 때리되 독자를 적으로 만들지 않는다.
- 자청·역행자 등 특정 작가의 '단어'는 그대로 쓰지 말 것. 스타일(톤)만 가져온다.
- 자극을 위해 어법을 깨지 마라. 아래 [자연스러움 원칙]이 항상 우선한다.
...
- 추상 개념 + 기계·물리 동사 금지: 재투자·수익·복리·멘탈·습관·시스템 같은 추상 개념에 "켜다/끄다/돌리다/감다/조이다/풀다/꽂다/심다/굴린다/얹는다/멈춘다" 같은 기계·물리 동사를 붙이지 마라. (추상 개념은 스위치가 아니다)
  ❌ "재투자를 켠 사람과 끈 사람" (재투자는 켜고 끄는 게 아님 — 말이 안 됨)
  ✅ "수익을 다시 넣은 사람과 빼서 쓴 사람의 3년 뒤" (대비는 살리되 어법은 자연스럽게)
