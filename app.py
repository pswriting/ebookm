...
.stApp {
    background:
        radial-gradient(ellipse 80% 50% at 50% -5%, rgba(201,162,75,0.07) 0%, transparent 60%),
        radial-gradient(ellipse 60% 50% at 100% 100%, rgba(201,162,75,0.04) 0%, transparent 55%),
        #08080A !important;
...
/* ============================================================
   PREMIUM REDESIGN LAYER — 전문가급 공통 디자인 (모든 화면 적용)
   ============================================================ */

/* 본문 영역 여백/리듬 — 좁고 촘촘하게 (이미지처럼 카드형 밀도) */
.main .block-container,
[data-testid="stMainBlockContainer"] { max-width: 680px !important; padding: 1.4rem 1.5rem 3rem !important; }
/* 요소 사이 휑한 여백 압축 */
[data-testid="stVerticalBlock"] { gap: 0.55rem !important; }
[data-testid="stHorizontalBlock"] { gap: 0.6rem !important; }
.element-container { margin-bottom: 0 !important; }
[data-testid="stMarkdownContainer"] > p { margin-bottom: 0.4rem !important; }
hr { margin: 18px 0 !important; }
/* 섹션 타이틀 박스 더 콤팩트 */
.section-title-box { padding: 22px 26px !important; margin-bottom: 18px !important; }

/* 타이포 리듬 정돈 */
h1, h2, h3, h4, h5 { font-family: 'S-CoreDream','Pretendard',sans-serif !important; }
h4 { font-size: 17px !important; font-weight: 400 !important; color: var(--cream) !important; letter-spacing: 0.3px; }
h5 { font-size: 12px !important; font-weight: 500 !important; color: var(--gold) !important; letter-spacing: 0.18em !important; text-transform: uppercase; }
.main p, .main li { color: #C8C5BD !important; }

/* 폼 라벨 — 작은 골드 트래킹 라벨 */
.stTextInput label, .stTextArea label, .stNumberInput label, .stSelectbox label, .stRadio > label, .stCheckbox label p {
...
.stCaption, [data-testid="stCaptionContainer"], [data-testid="stCaptionContainer"] p {
    color: var(--text3) !important; font-size: 12px !important; letter-spacing: 0.04em;
}

/* ── 사이드바: 미니멀 프리미엄 패널 ── */
[data-testid="stSidebar"] {
    background: #0E0E10 !important;
    border-right: 0.5px solid var(--line2) !important;
}
...
[data-testid="stSidebar"] hr { margin: 18px 0 !important; background: var(--line2) !important; }
/* 사이드바 버튼은 채워진 골드 대신 미니멀 아웃라인 */
...
/* ── 라디오: 카드형 옵션 ── */
.stRadio > div { background: transparent !important; border: none !important; padding: 0 !important; gap: 8px !important; }
.stRadio > div > label {
    background: var(--card) !important;
    border: 0.5px solid var(--line2) !important;
    border-radius: 10px !important;
...
.stRadio > div > label:hover { border-color: var(--gold) !important; background: rgba(201,162,75,0.06) !important; }

/* ── 체크박스 ── */
.stCheckbox { padding: 4px 0; }

/* ── 탭: 깔끔한 언더라인 ── */
.stTabs [data-baseweb="tab"] { font-size: 14px !important; letter-spacing: 0.02em; color: var(--text2) !important; padding: 10px 4px !important; }
.stTabs [aria-selected="true"] { font-weight: 500 !important; }

/* ── 진행바: 얇고 우아하게 ── */
.stProgress > div > div { height: 4px !important; }
.stProgress > div > div > div { height: 4px !important; }

/* ── 다운로드/링크 버튼: 사이드바 안에서는 아웃라인 ── */
[data-testid="stSidebar"] .stDownloadButton button, [data-testid="stSidebar"] .stLinkButton a {
    background: rgba(255,255,255,0.03) !important;
    color: var(--gold) !important; -webkit-text-fill-color: var(--gold) !important;
    border: 0.5px solid var(--line) !important;
    border-radius: 10px !important;
    font-weight: 500 !important; letter-spacing: 0.03em !important; box-shadow: none !important;
}

/* 링크버튼 본문(메인) — 골드 아웃라인 알약 */
.stLinkButton a {
    border-radius: 12px !important;
    border: 0.5px solid var(--line) !important;
    letter-spacing: 0.03em !important;
...
/* metric/숫자 강조 */
[data-testid="stMetricValue"] { color: var(--gold-light) !important; font-weight: 300 !important; }
[data-testid="stMetricLabel"] { color: var(--text3) !important; letter-spacing: 0.1em; text-transform: uppercase; font-size: 11px !important; }

/* 데이터프레임/코드블록 라운드 */
.stDataFrame, .stCodeBlock, pre { border-radius: 12px !important; border: 0.5px solid var(--line2) !important; }

/* ============================================================
   FINAL FIX — 빨간 primary 버튼 제거 + 다크 입력칸 (이미지 일치)
   ============================================================ */

/* primary / form submit 버튼 → 골드 그라데이션 (빨강 제거) */
.stButton > button[kind="primary"],
.stButton > button[data-testid="baseButton-primary"],
[data-testid="stBaseButtonPrimary"],
...
[data-baseweb="base-input"] {
    ...
    border: 0.5px solid rgba(201,162,75,0.4) !important;
    border-radius: 10px !important;
}
...
input::placeholder, textarea::placeholder {
    ...
    -webkit-text-fill-color: #6E6B64 !important;
}

/* 셀렉트박스(닫힌 상태) 다크 + 골드 보더 */
.stSelectbox [data-baseweb="select"] > div {
    background: rgba(255,255,255,0.04) !important;
    border: 0.5px solid rgba(201,162,75,0.4) !important;
    border-radius: 10px !important;
...
# ==========================================
# 비디오 배경 헤더
# (헤더는 콤팩트 브랜드 히어로로 대체됨 — 비디오 배경 미사용)

# 콤팩트 브랜드 히어로 (이미지 1 스타일) — 거대한 비디오 박스 제거
st.markdown("""
<div style="display:flex;align-items:center;justify-content:space-between;padding:14px 4px;border-bottom:0.5px solid rgba(255,255,255,0.06);margin-bottom:8px;">
    <div style="display:flex;align-items:baseline;gap:10px;">
        <span style="font-size:17px;font-weight:500;letter-spacing:0.18em;color:#F5F3EF;">WRITEY</span>
        <span style="font-size:10px;letter-spacing:0.28em;color:#7A776F;">CASHMAKER</span>
    </div>
    <span style="font-size:11px;letter-spacing:0.1em;color:#7A776F;">남현우 작가</span>
</div>
<div style="text-align:center;padding:26px 20px 20px;margin-bottom:6px;">
    <div style="font-size:11px;letter-spacing:0.4em;color:#C9A24B;margin-bottom:12px;">A I&nbsp;&nbsp;G H O S T W R I T E R</div>
    <h1 style="font-family:'S-CoreDream','Pretendard',sans-serif;font-size:38px;font-weight:300;letter-spacing:0.14em;color:#FAF8F4;margin:0;">WRITEY</h1>
    <div style="width:34px;height:1px;background:#C9A24B;margin:15px auto 0;"></div>
    <p style="color:#8A8780;font-size:14px;margin-top:13px;font-weight:300;">6개의 질문에 답하면 AI가 목차부터 본문까지 완성합니다</p>
</div>
""", unsafe_allow_html=True)
...
if not st.session_state.get('interview_completed', False):
    ...
    if step == 1:
        st.markdown("""
        <div style="background:rgba(255,255,255,0.025);padding:18px 22px;border-radius:12px;border-left:2px solid var(--gold);margin-bottom:14px;">
    ...
    # ========== STEP 6: 목차 확인 및 본문 생성 ==========
        ...
        if outline:
            st.markdown('<div style="font-size:11px;letter-spacing:0.22em;color:#C9A24B;margin:8px 0 4px;">STEP 02 · 목차</div>', unsafe_allow_html=True)
            st.markdown('<div style="font-size:13px;color:#8A8780;margin-bottom:18px;">아래 카드는 미리보기입니다 · \'✏️ 수정\'을 펼치면 제목·소제목을 직접 고칠 수 있어요</div>', unsafe_allow_html=True)
            ...
            for i, ch in enumerate(outline):
                ...
                # 소제목 번호 리스트 (01. 02. ...) — 이미지 2 스타일
                _items = []
                for sub in subtopics:
                    _sub_counter += 1
                    _items.append(f"{_sub_counter:02d}. {html.escape(str(sub))}")
                _list_html = "<br>".join(_items) if _items else "<span style='color:#5C5A55;'>소제목 없음</span>"

                # PART 미리보기 카드 (골드 세로선)
                st.markdown(f"""
                <div style="border-left:1px solid rgba(201,162,75,0.5); padding:6px 0 6px 20px; margin:16px 0 6px;">
                    <div style="display:flex; align-items:baseline; gap:12px; flex-wrap:wrap;">
                        <span style="font-size:11px; letter-spacing:0.2em; color:#C9A24B;">PART {i+1}</span>
                        <span style="font-size:17px; font-weight:500; color:#FAF8F4;">{html.escape(str(ch))}</span>
                    </div>
                    <div style="font-size:13px; color:#8A8780; margin-top:10px; line-height:1.95;">{_list_html}</div>
                </div>
                """, unsafe_allow_html=True)

                # 편집 패널 (펼치기)
                with st.expander(f"✏️ PART {i+1} 수정"):
...
# ==========================================
    ...
    # ── 완성 통계 계산 (이미지 4 스타일 히어로용) ──
    _outline7 = st.session_state.get('outline', [])
    _chapters7 = st.session_state.get('chapters', {})
    _full_text7 = ""
    for _ch7 in _outline7:
        _cd7 = _chapters7.get(_ch7, {})
        for _s7 in _cd7.get('subtopics', []):
            _full_text7 += (_cd7.get('subtopic_data', {}).get(_s7, {}).get('content', '') or "")
    _char7 = len(_full_text7.replace(' ', '').replace(chr(10), ''))
    _nchap7 = len(_outline7)
    _npage7 = max(1, _char7 // 500)
    _btitle7 = html.escape(st.session_state.get('book_title', '') or '제목 미정')
    _bsub7 = html.escape(st.session_state.get('subtitle', '') or '')
    _bauthor7 = html.escape(st.session_state.get('author_name', '') or st.session_state.get('interview_data', {}).get('author_name', '') or '저자')

    st.markdown(f"""
    <div style="text-align:center; padding:24px 20px 12px;">
        <div style="font-size:11px; letter-spacing:0.4em; color:#C9A24B; margin-bottom:14px;">M A N U S C R I P T&nbsp;&nbsp;R E A D Y</div>
        <div style="font-size:28px; font-weight:300; color:#FAF8F4; line-height:1.4;">당신의 책이 <span style="font-weight:500;">완성</span>되었습니다</div>
        <div style="width:34px; height:1px; background:#C9A24B; margin:18px auto 0;"></div>
    </div>
    <div style="margin:14px 0 8px; padding:26px; background:rgba(255,255,255,0.025); border:0.5px solid rgba(201,162,75,0.2); border-radius:14px; display:flex; gap:22px; align-items:center; flex-wrap:wrap;">
        <div style="width:90px; height:124px; flex-shrink:0; border-radius:6px; background:linear-gradient(150deg,#1A1A1F,#0B0B0D); border:0.5px solid rgba(201,162,75,0.35); display:flex; flex-direction:column; justify-content:center; align-items:center; padding:10px; text-align:center;">
            <span style="font-size:9px; letter-spacing:0.15em; color:#C9A24B; margin-bottom:8px;">CASHMAKER</span>
            <span style="font-size:11px; font-weight:500; color:#F5F3EF; line-height:1.4;">{_btitle7}</span>
            <span style="font-size:8px; color:#7A776F; margin-top:8px;">{_bauthor7} 지음</span>
        </div>
        <div style="flex:1; min-width:200px;">
            <div style="font-size:18px; font-weight:500; color:#FAF8F4; line-height:1.4;">{_btitle7}</div>
            <div style="font-size:13px; color:#8A8780; margin-top:6px;">{_bsub7}</div>
            <div style="display:flex; gap:26px; margin-top:18px; flex-wrap:wrap;">
                <div><div style="font-size:21px; font-weight:300; color:#E0C074;">{_nchap7}</div><div style="font-size:10px; color:#7A776F; letter-spacing:0.08em;">CHAPTERS</div></div>
                <div><div style="font-size:21px; font-weight:300; color:#E0C074;">{_npage7}p</div><div style="font-size:10px; color:#7A776F; letter-spacing:0.08em;">PAGES</div></div>
                <div><div style="font-size:21px; font-weight:300; color:#E0C074;">{_char7:,}</div><div style="font-size:10px; color:#7A776F; letter-spacing:0.08em;">글자수</div></div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
