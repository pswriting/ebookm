...
목차의 '의미·자극·호기심·구성'은 그대로 두고, 오직 **어색한 한국어 표현만** 자연스럽게 고쳐라.
{lock}
...
[교정 원칙]
- 한 줄 한 줄 소리 내어 읽었다고 가정하고, 어색하면 같은 의도·같은 자극을 유지한 채 자연스러운 한국어로 다시 쓴다.
- 자연스러우면 그대로 둔다(억지로 바꾸지 말 것). 의미를 바꾸거나 밋밋하게 만들지 말 것.
...
체크 15. (최종 결제 테스트) 완성된 목차를 처음부터 끝까지 5초간 훑었다고 상상하라. "이 목차 정도는 나도 알겠다" 싶으면 실패다. "이건 안 읽으면 못 배기겠다" 싶을 때까지, 가장 약한 5줄을 골라 충격·호기심을 더 키워 다시 써라.
...
목차만 출력. 콜론 금지. 매 소제목 다른 패턴. 명사·동사 엔딩 섞기. 어법 어색한 표현 즉시 폐기. 각 PART는 정확히 소제목 4개. 컨셉명 글자수 단정 금지. 자가점검 결과는 출력하지 말 것."""
    _raw = ask_ai(prompt, 0.85, ensure_quality=True)
