...
def _cover_font(candidates, size):
    """후보 경로들 중 존재하는 폰트를 로드. 실패 시 기본 폰트."""
...
def render_cover_png(path, title, subtitle, author, brand):
    ...
    try:
        ...
        if subtitle:
            centered(sep_y + 40, subtitle, _cover_font(SERIF_MED, 33), SUBC, tracking=2)

        d.line([(cx - 30, H - 290), (cx + 30, H - 290)], fill=GOLD, width=1)
        centered(H - 250, author or "저자", _cover_font(SANS, 34), AUTHC, tracking=8)
        centered(H - 196, "지음", _cover_font(SANS, 22), DIM, tracking=4)
...
- 비유는 '명사'로만 쓰고 '되다/안 되다'의 대상으로 삼지 마라(비유 오용 금지). "월세"는 받는 금액의 비유지, "월세가 된다/안 된다"처럼 술어의 주어가 될 수 없다. 어색하면 비유를 버리고 '구체 금액'으로 직진하라 — 이게 더 강력하다.
  ...
  ✅ "배당주 3년을 모아도 매달 통장에 30만원도 안 꽂히는 사람들의 공통점" (구체 금액·장면)
